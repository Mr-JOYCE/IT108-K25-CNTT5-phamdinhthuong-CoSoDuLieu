...
-- [BÀI TẬP TỔNG HỢP] TỰ ĐỘNG HÓA VÍ TRẢ TRƯỚC

-- Phần A: Phân tích kiến trúc Trigger

/*
Sự kiện kích hoạt:
  - Thời điểm: BEFORE INSERT — vì phải kiểm tra ví và đơn giá trước khi ghi nhận dòng
    Service_Usages; đồng thời có thể gán NEW.actual_price trước khi INSERT được lưu.
  - Sự kiện: INSERT ON Service_Usages — đúng quy trình “chỉ định dịch vụ mới”.

Biến cục bộ (DECLARE) gợi ý:
  - v_service_price   — hứng đơn giá từ Services.price (theo NEW.service_id).
  - v_wallet_balance  — hứng số dư hiện tại Wallets.balance (theo NEW.patient_id).
  - v_wallet_status   — hứng trạng thái ví Wallets.status ('Active' | 'Inactive').
*/

-- Phần B: Triển khai & kiểm thử
...
FOR EACH ROW
BEGIN
    DECLARE v_service_price DECIMAL(18, 2);
    ...
    SELECT w.balance, w.status INTO v_wallet_balance, v_wallet_status
    FROM Wallets AS w
    ...
    IF v_wallet_balance < v_service_price THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Thất bại: Số dư ví không đủ để thanh toán';
    END IF;

    SET NEW.actual_price = v_service_price;

    UPDATE Wallets AS w
    SET w.balance = w.balance - v_service_price
    WHERE w.patient_id = NEW.patient_id;
END //

DELIMITER ;

-- Kiểm thử — chỉ truyền patient_id và service_id (actual_price do trigger gán + trừ ví)

-- Kịch bản 1: Giao dịch hợp lệ (BN 1, ví Active 500k; dịch vụ 2 = 150k)
INSERT INTO Service_Usages (patient_id, service_id) VALUES (1, 2);

-- Chứng minh đã trừ tiền: 500000 - 150000 = 350000
SELECT patient_id, balance, status FROM Wallets WHERE patient_id = 1;

-- Kịch bản 2: Thẻ khóa (BN 3, ví Inactive)
INSERT INTO Service_Usages (patient_id, service_id) VALUES (3, 1);

-- Kịch bản 3: Cháy ví (BN 2 còn 50k; dịch vụ 1 = 200k)
