...
-- [BÀI TẬP] SỰ CỐ DỜI LỊCH KHÁM VỀ QUÁ KHỨ

-- Phần A: Phân tích lỗi

-- 1) Câu lệnh UPDATE tái hiện lỗi (khi trigger SAI vẫn dùng OLD.appointment_date):
--    Lịch 104 đang ở tương lai (2026-06-10); đổi sang tháng 1 năm trước → quá khứ.
--    Trigger sai chỉ xét OLD (vẫn là ngày cũ trong tương lai) nên điều kiện
--    OLD < NOW() không đúng → không SIGNAL → giao dịch vẫn commit.

/*
UPDATE Appointments
SET appointment_date = '2025-01-15 08:30:00'
WHERE appointment_id = 104;
*/

-- 2) Giải thích ngắn gọn:
-- OLD.appointment_date là giá trị dòng đang có trước khi UPDATE; NEW.appointment_date
-- là giá trị sau khi gán trong lệnh UPDATE (ngày giờ “mới” cần kiểm tra).
-- Mã cũ so sánh ngày CŨ với NOW() nên không phát hiện khi ta đổi từ lịch tương lai
-- sang một mốc trong quá khứ.

-- Phần B: Sửa chữa — xóa trigger cũ và tạo lại đúng logic (kiểm tra NEW)
