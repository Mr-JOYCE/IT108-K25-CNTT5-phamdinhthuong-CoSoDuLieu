...
-- PHẦN A — Phân tích
--
-- 1) Câu lệnh CALL tái hiện lỗi (với thủ tục CŨ chỉ UPDATE theo appointment_id):
--    appointment_id = 105 trong DB mẫu trên có status = 'Completed'.
--    Sau khi gọi, nếu thủ tục lỗi thì dòng này vẫn bị SET status = 'Cancelled'.
--
--    CALL CancelAppointment(105);
--
--    Kiểm tra: SELECT appointment_id, status FROM Appointments WHERE appointment_id = 105;
--
-- 2) Giải thích ngắn (2–3 dòng):
--    Thủ tục cũ chỉ điều kiện WHERE theo appointment_id, không kiểm tra status hiện tại.
--    Do đó mọi trạng thái (Completed, Cancelled, …) đều bị ghi đè thành 'Cancelled',
--    trái quy tắc chỉ cho phép hủy khi lịch đang 'Pending'.

-- PHẦN B — Sửa chữa: xóa thủ tục cũ và tạo lại với điều kiện chỉ hủy khi Pending
