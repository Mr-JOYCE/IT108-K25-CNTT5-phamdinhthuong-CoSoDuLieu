...
-- PHẦN 3: BÀI TẬP - SỰ CỐ GIAO DỊCH CHƯA HOÀN TẤT

-- PHẦN A: PHÂN TÍCH

-- A.1. Thủ tục hiện tại (thiếu cơ chế bảo vệ giao dịch)
DELIMITER //
...
-- A.2. Kiểm tra dữ liệu TRƯỚC khi gọi thủ tục (bệnh nhân 1)
...
-- A.3. Tái hiện lỗi: gọi thanh toán 200.000đ (thủ tục sẽ báo lỗi và dừng giữa chừng)
CALL PayHospitalFee(1, 200000.00);

-- A.4. Kiểm tra dữ liệu SAU khi gọi thủ tục (quan sát trạng thái lệch)
SELECT patient_id, balance FROM Wallets WHERE patient_id = 1;
SELECT patient_id, total_due FROM Patient_Invoices WHERE patient_id = 1;

/*
A.5. Giải thích ACID (2–3 dòng):

Sự cố "tiền đã trừ nhưng nợ không giảm" vi phạm tính Atomicity (Nguyên tử) của Transaction:
toàn bộ các bước trong một giao dịch phải cùng thành công hoặc cùng bị hủy, không được
dừng giữa chừng. Đồng thời cũng làm sai lệch Consistency (Nhất quán) vì tổng tiền ví
và công nợ không còn khớp với quy tắc nghiệp vụ của phòng khám.
*/

-- Khôi phục dữ liệu mẫu trước khi sửa (chạy sau khi đã tái hiện lỗi ở A.3)
...
BEGIN
    ...
    -- Giả lập sự cố (để kiểm chứng ROLLBACK hoạt động)
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Lỗi: Hệ thống gặp sự cố mạng đột ngột!';
...
-- Kiểm chứng: gọi lại thủ tục đã sửa — ví và công nợ phải giữ nguyên sau lỗi
CALL PayHospitalFee(1, 200000.00);
